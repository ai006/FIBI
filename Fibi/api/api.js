...
import {fetchJobTypesPending, fetchJobTypesError, fetchJobTypesSuccess} from '../redux/actions/indexJobTypes';
...
//function used to call and fetch all the type of jobs 
//in our database from the server
export const fetchJobType =  () => {
  return async (dispatch) => {
    try{
      dispatch(fetchJobTypesPending())
      const response = await fetch(API_Link + 'typesOfJobs');
      const responseJson = await response.json();
      if(responseJson.message !== undefined){
        throw(responseJson.message)
      }
      dispatch(fetchJobTypesSuccess(responseJson.data))
    }catch(error){
      //console.log(error);
      dispatch(fetchJobTypesError(error))
    }
  }
}
